...
def preprocess_by_day(
    ...
    pivot_df = input_df.pivot_table(
        index=uid_col, columns=date_col, values=quantity_col, fill_value=0
    )

    # Reindex the pivot table to fill in missing dates with 0s
    pivot_df = pivot_df.reindex(
        columns=pd.date_range(
            start=input_df[date_col].min(), end=input_df[date_col].max()
        ),
        fill_value=0,
    )
    ...
    grouped_df = (
        input_df.drop([price_col], axis=1)
        .groupby([uid_col, date_col])
    ...
    most_common_price = (
        input_df.groupby([uid_col, date_col])[price_col]
    ...
    return (
        grouped_df.merge(most_common_price, on=[uid_col, date_col])
        .reset_index()
...
def preprocess_by_price(
    ...
    Args:
        ...
        date_col (str, optional): The column name for the date. Defaults to "date".
        price_col (str, optional): The column name for the price. Defaults to "price".
        quantity_col (str, optional): The column name for the quantity. Defaults to "units".
        weights_col (str, optional): The column name for the weights. Defaults to "days".

    Returns:
        tuple[pd.DataFrame, pd.DataFrame]: A tuple containing two DataFrames:
            - df_by_price_norm: The preprocessed DataFrame grouped by price normalization.
            - df_by_price_norm: The preprocessed DataFrame with renamed columns.
...
def uid_with_min_conversions(
    ...
    return (
        ...
        .groupby(uid_col)
        .filter(lambda x: len(x) >= min_days_with_conversions)[uid_col]
        .unique()
    )

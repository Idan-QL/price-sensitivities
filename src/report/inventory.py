...
def get_report(bucket, sensitivity_parquet):
    results = []

    df_s = pd.read_parquet(sensitivity_parquet)
    df_report = pd.read_csv(f's3://{bucket}/data_science/eval_results/elasticity/elasticity_report_2024-03-01.csv')
    sensitivity_client = pd.concat([df_report[['client_key', 'channel']],df_s[['client_key', 'channel']]]).groupby(['client_key', 'channel']).size().reset_index()[['client_key', 'channel']]

    for client_key, channel in zip(sensitivity_client['client_key'].tolist(), sensitivity_client['channel'].tolist()):
        df_s_client = df_s[(df_s['client_key'] == client_key) & (df_s['channel'] == channel)]
        uid_sensitivity = df_s_client['uid'].tolist()
        try:
            df_e_client = pd.read_csv(f's3://{bucket}/data_science/eval_results/elasticity/elasticity_{client_key}_{channel}_2024-03-01.csv')
    ...
    df_report_merge['sensitivity_from_total'] = round(df_report_merge['uid_with_sensitivity']*100 / df_report_merge['total_uid'],2)
    df_report_merge['both_from_total'] = round(df_report_merge['uid_with_both']*100 / df_report_merge['total_uid'],2)
    df_report_merge['at_least_one_from_total'] = round(df_report_merge['uid_with_at_least_one']*100 / df_report_merge['total_uid'],2)
    df_report_merge.rename(columns={'uids_from_total': 'elasticity_from_total', 'uids_from_total_with_data': 'elasticity_from_total_with_data',}, inplace=True)

    df_inventory = df_report_merge[['client_key', 'channel', 'total_uid', 'uid_with_both','uid_with_at_least_one', 'uid_with_elasticity',
        'uid_with_sensitivity', 'both_from_total' ,'at_least_one_from_total', 'elasticity_from_total', 'elasticity_from_total_with_data', 'sensitivity_from_total']]
